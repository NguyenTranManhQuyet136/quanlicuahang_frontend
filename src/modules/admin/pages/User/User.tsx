...
const User = () => {
    ...
    useEffect(() => {
        const getDataAccount = async () => {
            ...
            try {
                ...
                const profile = res.data && res.data.length > 0 ? res.data[0] : null;
    ...
    const changeAvatar = async (file: any) => {
        ...
        try {
            await axios.post("http://localhost:5000/api/upload_avatar", formData, {
                headers: {
    ...
    const closeForm = (type: string) => {
        if (type === "pass") setChangePasswordStatus(false);
