...
const Product = () => {
    ...
    const dynamicColInfo = colInfo.map(col => {
        ...
        if (col.key === "type") {
            const uniqueCategories = Array.from(new Set(dataProduct.map(p => p.type)));
            return {
                ...col,
    ...
    const closeForm = (type: any) => {
        switch (type) {
            case "remove":
                setRemoveStatus({ status: false, product_id: "", name: "" });
                break;
            case "fix":
                setFixStatus({ statusSwitch: false, dataFix: {} });
                break;
            case "add":
                setAddStatus({ status: false });
                break;
            case "search":
                setSearchStatus({ status: false, product_id: "", name: "" });
                break;
            default:
                break;
        }
    ...
    const handleRemove = async (product_id: any) => {
    ...
    const handleFix = async (dataFix: any, idOld: any) => {
    ...
    const uniqueTypes = Array.from(new Set(dataProduct.map((item) => item.type)));
                    ...
                    <div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                            ...
                            <div style={{ display: "flex", gap: "8px" }}>
                                <button style={{ display: "flex", alignItems: "center", gap: "6px", padding: "8px 16px", backgroundColor: "#0d6efd", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: "500", fontSize: "0.95rem" }} onClick={() => setSearchStatus({ status: true, product_id: "", name: "" })}>{(FiSearch as any)({ size: 18 })}Tìm kiếm</button>
                                <button style={{ display: "flex", alignItems: "center", gap: "6px", padding: "8px 16px", backgroundColor: "#0d6efd", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: "500", fontSize: "0.95rem" }} onClick={() => setAddStatus({ status: true })}>{(FiPlus as any)({ size: 18 })}Thêm sản phẩm</button>
                                <button style={{ display: "flex", alignItems: "center", gap: "6px", padding: "8px 16px", backgroundColor: "#198754", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: "500", fontSize: "0.95rem" }} onClick={() => window.location.reload()}>{(FiRefreshCw as any)({ size: 18 })}Tải lại</button>
                            </div>
                        </div>
